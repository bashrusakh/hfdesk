...
func TestAnalyzeGGUF(t *testing.T) {
	...
	t.Run("split GGUF shards are merged by quantization", func(t *testing.T) {
		files := []FileInfo{
			...
			{Name: "model-Q4_K_M-00001-of-00002.gguf", Path: "model-Q4_K_M-00001-of-00002.gguf", Size: 2000, SizeHuman: "2.0 KiB"},
			{Name: "model-Q5_K_M.gguf", Path: "model-Q5_K_M.gguf", Size: 7000, SizeHuman: "6.8 KiB"},
		}
		...
		var q4 *GGUFQuantization
		for i := range result.Quantizations {
			if result.Quantizations[i].Name == "Q4_K_M" {
				q4 = &result.Quantizations[i]
				break
			}
		}
		if q4 == nil {
			t.Fatal("expected Q4_K_M quantization")
		}
		if q4.File.Size != 5000 {
			t.Errorf("File.Size = %d, want 5000", q4.File.Size)
		}
		if q4.Parts != 2 {
			t.Errorf("Parts = %d, want 2", q4.Parts)
		}
		if len(q4.Files) != 2 {
			t.Fatalf("Files length = %d, want 2", len(q4.Files))
		}
		if q4.Files[0] != "model-Q4_K_M-00001-of-00002.gguf" || q4.Files[1] != "model-Q4_K_M-00002-of-00002.gguf" {
			t.Errorf("Files = %#v, want sorted shard paths", q4.Files)
		}
	})

	t.Run("split GGUF shards use quantization from parent path", func(t *testing.T) {
		files := []FileInfo{
			{Name: "model-00001-of-00002.gguf", Path: "UD-Q3_K_XL/model-00001-of-00002.gguf", Size: 1000, SizeHuman: "1000 B"},
			{Name: "model-00002-of-00002.gguf", Path: "UD-Q3_K_XL/model-00002-of-00002.gguf", Size: 1500, SizeHuman: "1.5 KiB"},
		}
		...
		if quant.Parts != 2 {
			t.Errorf("Parts = %d, want 2", quant.Parts)
		}
		if quant.Name == "Unknown" {
			t.Error("quantization should not be Unknown when the parent path contains a quant token")
		}
	})
}
...
func TestGGUFToSelectableItems(t *testing.T) {
	...
	t.Run("sharded quantization includes every shard", func(t *testing.T) {
		info := &GGUFInfo{
			Quantizations: []GGUFQuantization{
				{
					Name:        "Q3_K_XL",
					Quality:     3,
					Description: "Unsloth dynamic 3-bit, improved quality",
					File: FileInfo{
						Path:      "UD-Q3_K_XL/model-00001-of-00002.gguf",
						Size:      2500,
						SizeHuman: "2.4 KiB",
					},
					Parts: 2,
					Files: []string{
						"UD-Q3_K_XL/model-00001-of-00002.gguf",
						"UD-Q3_K_XL/model-00002-of-00002.gguf",
					},
				},
			},
		}
		...
		item := items[0]
		if item.FilterValue != "q3_k_xl" {
			t.Errorf("FilterValue = %q, want q3_k_xl", item.FilterValue)
		}
		if len(item.Files) != 2 {
			t.Fatalf("Files length = %d, want 2", len(item.Files))
		}
		if item.Files[0] != "UD-Q3_K_XL/model-00001-of-00002.gguf" || item.Files[1] != "UD-Q3_K_XL/model-00002-of-00002.gguf" {
			t.Errorf("Files = %#v, want all shard paths", item.Files)
		}
		if !strings.Contains(item.Description, "2 files") {
			t.Errorf("Description = %q, want shard count", item.Description)
		}
	})
}
